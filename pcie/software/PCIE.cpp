...
bool __fastcall PCIE::LoadDll(void){
	...
	fnIsWow64Process = (LPFN_ISWOW64PROCESS)GetProcAddress( GetModuleHandle((const wchar_t *)"kernel32"),"IsWow64Process");
	...
	if(bIsWow64)
	{
		m_hDLL = ::LoadLibrary((const wchar_t *)"TERASIC_PCIEx64.DLL");
	}
	else
	{
		m_hDLL = ::LoadLibrary((const wchar_t *)"TERASIC_PCIE.dll");
	}
